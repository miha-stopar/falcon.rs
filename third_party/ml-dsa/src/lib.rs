...
impl<P: MlDsaParams> Signature<P> {
    /// Raw ML-DSA challenge hash bytes `c_tilde` (serialized prefix of the signature).
    #[inline]
...
impl<P: MlDsaParams> VerifyingKey<P> {
    ...
    /// Domain-separated μ matching [`Self::verify_internal`] (Algorithm 8 preprocessing).
    ///
    /// Exposed for ZKP witness pipelines that must reuse the same μ as ML-DSA.Verify_internal.
    pub fn mu_for_verify_internal(&self, M: &[u8]) -> B64 {
        MuBuilder::internal(&self.tr, &[M])
    }
    ...
    /// Recompute `c_tilde` from FIPS 204 **ML-DSA.Verify_internal** (challenge hash step).
    ///
    /// This is a **non-standard** helper for ZKP witness generation: it runs the same arithmetic
    /// as [`Self::raw_verify_mu`] but returns the squeezed challenge bytes instead of comparing to
    /// `sigma.c_tilde`.
    pub fn recomputed_challenge_bytes(&self, mu: &B64, sigma: &Signature<P>) -> Array<u8, P::Lambda>
    where
        P: MlDsaParams,
    {
        let c = sample_in_ball(&sigma.c_tilde, P::TAU);
